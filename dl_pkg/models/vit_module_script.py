...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_size=768, num_heads=8, dropout=0.0):
        ...
        self.key = nn.Sequential(
            ...
            Rearrange("batch seq_len (num_head h_dim) -> batch num_head seq_len h_dim", num_head=num_heads)
        )
    ...
    def forward(self, x: torch.Tensor, mask : Optional[torch.Tensor] =None):
        ...
        if mask is not None:
            ...
            fill_value = 1e-20
            energies.masked_fill(~mask, fill_value)
...
class TransformerEncoderBlock(nn.Sequential):
    def __init__(
        self, emb_size=768, drop_p=0.0, forward_expansion=4, forward_drop_p=0.0, **kwargs
    ):
